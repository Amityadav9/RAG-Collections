...
def init_session_state():
    ...
    if 'ollama_model' not in st.session_state:
        st.session_state.ollama_model = "llama3.2:3b"  # You can change to any model you have
    if 'ollama_embedding_model' not in st.session_state:
        st.session_state.ollama_embedding_model = "nomic-embed-text:latest"  # Popular embedding model for Ollama
    if 'qdrant_url' not in st.session_state:
        st.session_state.qdrant_url = "http://localhost:6333"  # Default local Qdrant port
    if 'qdrant_api_key' not in st.session_state:
        st.session_state.qdrant_api_key = ""  # Local Qdrant usually doesn't need API key
...
def initialize_models():
    """Initialize Ollama models and Qdrant client"""
    if st.session_state.ollama_base_url and st.session_state.qdrant_url:
        try:
            # Initialize Ollama embeddings
            st.session_state.embeddings = OllamaEmbeddings(
                model=st.session_state.ollama_embedding_model,
            ...
            try:
                ...
                # Test LLM
                test_response = st.session_state.llm.invoke("Say 'Hello'")
            except Exception as e:
                st.error(f"Failed to connect to Ollama: {str(e)}")
                st.info("Make sure Ollama is running and the models are pulled:")
                st.code(f"ollama pull {st.session_state.ollama_model}")
                st.code(f"ollama pull {st.session_state.ollama_embedding_model}")
                return False
...
def create_routing_agent() -> Agent:
    """Creates a routing agent using agno framework with Ollama"""
    return Agent(
        model=Ollama(
            id=st.session_state.ollama_model
        ),
        tools=[],
...
def route_query(question: str) -> Optional[DatabaseType]:
    ...
    try:
        ...
        # Check if the returned db_type is valid
        valid_db_types = list(COLLECTIONS.keys())
...
def create_fallback_agent(chat_model: BaseLanguageModel):
    """Create a simple web search function instead of LangGraph agent."""
...
def _handle_web_fallback(question: str) -> tuple[str, list]:
    ...
    web_search_func = create_fallback_agent(st.session_state.llm)
    
    with st.spinner('Researching...'):
        try:
            # Get web search results
            search_results = web_search_func(question)
            
            # Use the LLM to synthesize an answer from search results
            prompt = f"""Based on the following web search results, provide a comprehensive answer to the question: "{question}"

Web search results:
{search_results}

Please provide a clear, informative answer based on these search results:"""
            ...
            fallback_response = st.session_state.llm.invoke(question)
            answer = fallback_response.content if hasattr(fallback_response, 'content') else str(fallback_response)
            return f"Web search unavailable. General response: {answer}", []

def main():
    """Main application function."""
    st.set_page_config(page_title="Local RAG Agent with Database Routing", page_icon="📚")
    st.title("📠 Local RAG Agent with Database Routing")
    ...
    with st.sidebar:
        ...
        ollama_model = st.text_input(
            ...
            help="Examples: 3.2:3b, mistral, mixtral, etc."
        )
        
        ollama_embedding_model = st.text_input(
            "Ollama Embedding Model:",
            value=st.session_state.ollama_embedding_model,
            help="Examples: nomic-embed-text:latest, all-minilm, etc."
        )
        ...
        qdrant_url = st.text_input(
            ...
            help="Default for local: http://localhost:6333"
        )
        ...
        qdrant_api_key = st.text_input(
            ...
            help="Usually not needed for local Qdrant"
        )
        ...
        if ollama_embedding_model:
            st.session_state.ollama_embedding_model = ollama_embedding_model
        if qdrant_url:
        ...
        if st.button("Initialize Models"):
            with st.spinner("Initializing..."):
                if initialize_models():
                    st.success("Connected to Ollama and Qdrant successfully!")
                else:
                    st.error("Failed to initialize. Please check your services are running.")
    ...
    if not st.session_state.embeddings or not st.session_state.llm:
        st.warning("Please initialize models in the sidebar first!")
        st.stop()
