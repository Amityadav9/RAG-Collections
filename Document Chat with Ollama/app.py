...
@st.cache_resource
def initialize_llm():
    initialized_llm = Ollama(model="llama3.2:latest", request_timeout=120.0)
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if document_key not in st.session_state.get("doc_cache", {}):
                    if os.path.exists(temp_dir):
                        ...
                        st.error(
                            "Unable to find the uploaded file, please check again..."
                        )
                        st.stop()
...
if user_input := st.chat_input("Hi There?"):
    ...
    st.session_state.chat_messages.append(
        {"role": "assistant", "content": response_accum}
    )
